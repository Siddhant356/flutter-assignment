...
class EditDetail extends StatefulWidget {
  ...
  const EditDetail({Key? key, required this.employee}) : super(key: key);
...
class _EditDetailState extends State<EditDetail> {
  ...
  void initState() {
    ...
    startDateController.text = DateFormat('d MMM y').format(widget.employee.startDate);
    currentDate = widget.employee.startDate;
    endDateController.text = widget.employee.endDate != null ? DateFormat('d MMM y').format(widget.employee.endDate!) : '';
  ...
  Widget build(
    ...
    return Scaffold(
        ...
        appBar: boxAppBar('Edit Employee Details',
            icon: InkWell(
              onTap: () {
                employeeGlobalBloc.add(DeleteEmployeeEvent(employeeId: widget.employee.id));
